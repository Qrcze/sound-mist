...
public partial class App : Application
{
    ...
    private async void TrackItem_Like(object? sender, RoutedEventArgs e)
    {
        ...
        e.Handled = true;
        bool requestedState = !track.IsLiked;
        var (success, message) = await GetService<SoundCloudCommands>().SetTrackLiked(requestedState, track);
        if (success)
        {
        ...
        NotificationManager.Show(new Notification(
            requestedState ? "Failed liking track" : "Failed removing like",
            message,
